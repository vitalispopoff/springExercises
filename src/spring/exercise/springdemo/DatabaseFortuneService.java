...
//		if (fortuneData.size()==0) setFortuneData();
		...
		int index = myRandom.nextInt(fortuneData.size());
		String theFortune = fortuneData.get(index);

		return theFortune;
	}

	public void setFortuneData(String string) {
		fortuneData.add(string);
	}

	@PostConstruct
	public void loadFortuneData() {
		System.out.println(" ..." + this.getClass() + " : @postConstruct setFortuneData");
		String source = "src\\externalFortunes.txt";
